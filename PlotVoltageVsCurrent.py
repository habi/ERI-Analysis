...
for counter, FolderToPlot in enumerate(Folders):
    ...
    print '%s/%s: Reading values from %s images in folder %s' % (counter + 1,
        len(Folders), len(ImageList), os.path.basename(FolderToPlot))
    ...
    plt.savefig(os.path.join(os.path.expanduser('~'), 'Data20', 'CNT',
                             'ERI-Analysis', 'Images',
                             'VoltageVsCurrent_' + os.path.basename(FolderToPlot)
                             + '.png'))
    plt.xlim([20, 70])
    plt.ylim([0, 50])
    plt.savefig(os.path.join(os.path.expanduser('~'), 'Data20', 'CNT',
